...
3. **Branch: is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally (one red-green-refactor slice at a time), then closes out by running **`/code-review`**, a review of the diff against standards and specification using the repository's review policy, before committing. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point or review uncommitted work.
...
- **Something's broken** → **`/diagnosing-bugs`**. For the hard ones: the bug that resists a first glance, the intermittent flake, the regression that crept in between two known-good states. It inspects the relevant path to build a tight feedback signal, tests hypotheses as evidence requires, and fixes the supported cause with a meaningful regression or clearly stated verification limits.
...
- **`/improve-codebase-architecture`** audits a requested area for concrete friction. A report does not force an interview, refactor, or new task; **`/codebase-design`** helps with a selected interface choice.
...
- **`/grilling`** stress-tests a requested decision with manageable rounds of consequential questions. It does not require exhausting every possible branch.
...
- **`/research`** investigates a requested question using primary sources. It returns cited evidence; files and background research are conditional on the task.
