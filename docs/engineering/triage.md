...
`triage` works through the issues on your project's tracker, moving each one through repository-configured **triage roles** (a work type and a readiness state) and leaving behind either an agent-ready brief, a specific request for missing input, or a closed issue with a recorded reason.
...
Every item in triage resolves to exactly one work type and one readiness state. The four work types are `bug`, `feature`, `improvement`, and `documentation`. The six readiness states are:

| State | Means |
| --- | --- |
| `needs-triage` | You need to evaluate it. Where an unlabelled issue normally lands first. |
| `needs-information` | Waiting on the reporter. Returns to `needs-triage` when they reply. |
| `needs-evidence` | Waiting on a reproduction, measurement, source, or second observation. |
...
| `not-planned` | Closed or canceled, with the reason recorded. |

The actual tracker representation comes from configuration. On Linear, work types are labels, readiness is a status plus an optional qualifier label, and dependency edges are native issue relations. Execution statuses such as `In Progress`, `In Review`, and `Done` follow triage rather than competing with readiness roles.
...
| Rejected feature, improvement, or documentation request | A file in `.out-of-scope/`, linked from the closing comment, then close. |
...
Before any [grilling](https://www.aihero.dev/ai-coding-dictionary/grilling), `triage` checks that the claim actually holds. For a bug, it reproduces it from the reporter's steps. For a PR, it checks the branch out and runs the relevant tests. Then it reports which of three things happened: confirmed, failed to reproduce, or missing input. Missing reporter context maps to `needs-information`; a missing reproduction, measurement, source, or second occurrence maps to `needs-evidence`.

It runs two more checks against the codebase in the same pass: **redundancy** (is this already implemented, searched by domain concept rather than by the reporter's wording?) and **prior rejection** (does `.out-of-scope/` already say no?). Both are cheap, and both produce a `not-planned` outcome when they hit.
...
**The configured label or status does not exist in Linear.**
Re-run setup. It compares `issue-workflow.json` with the live tracker, previews every missing label, and asks before creating exact matches. Missing statuses must be added by a human or remapped when the integration cannot create them safely.

**How do blocked or already active issues fit?**
Blocking is a tracker relation, not a readiness label. A `ready-for-agent` issue with an open blocker is not on the frontier. Once claimed, the issue leaves triage readiness and moves through the configured execution statuses, such as `In Progress`, `In Review`, and `Done`.

**How is this different from `/diagnosing-bugs`?**
The verification step here is deliberately shallow (enough to answer "is this real, and roughly where does it live"), not to find a root cause. When a bug lacks a reproducible observation, the honest move is `needs-evidence`, or [diagnosing-bugs](https://aihero.dev/skills-diagnosing-bugs) if you want to chase it now.
...
Yes. Linear is a first-class template. A common split is Linear for issues and planning, GitHub for code and PRs: skills that say "issue tracker" map to Linear, while skills that say "PR" map to GitHub. Work types, readiness, projects, and relations stay in Linear and are not mirrored into GitHub Issues.
