...
The implementation route uses red-green-refactor, reviews the requested committed or uncommitted scope under repository policy, and treats diagnosis as evidence-led investigation rather than requiring a reproduction before inspection.
...
The tracker-dependent routes (triage, `to-spec`, `to-tickets`, `implement`) assume [setup-matt-pocock-skills](https://aihero.dev/skills-setup-matt-pocock-skills) has already configured an issue tracker plus its work-type and readiness mapping in the repo. Linear can own that workflow while GitHub remains the code host. The router will happily recommend these routes before setup has happened.
